...
local function Tween(obj, time, props)
    local success, tween = pcall(function()
        return TweenService:Create(obj, TweenInfo.new(time, Enum.EasingStyle.Linear), props)
    end)
...
local function TeleportToSea(seaNumber)
    ...
    if not placeIds[seaNumber] then
        warn("❌ Sea inválido!")
        return
    end
    
    if Sea == seaNumber then
        print("✅ Você já está no SEA " .. seaNumber)
        return
    end
    
    print("🚀 Iniciando teleport para SEA " .. seaNumber .. "...")
    
    -- Método 1: Teleport direto
    ...
    -- Método 2: Teleport com reserva de servidor
    ...
    if success2 then
        print("✅ Teleport iniciado (método 2)!")
        return
    end
    
    warn("❌ Falha ao teleportar. Tente novamente!")
end
...
-- =========================
-- BOLINHA (MOBILE FRIENDLY)
-- =========================
local Bubble = Instance.new("ImageButton", Gui)
Bubble.Size = UDim2.fromOffset(70, 70)
...
local function Toggle(parent, texto, callback)
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        if enabled then
            Tween(Circle, 0.2, {Position = UDim2.fromOffset(25, 2)})
            Tween(Btn, 0.2, {BackgroundColor3 = Config.Tema.Ativo})
        else
...
local function Dropdown(parent, texto, options, callback)
    ...
    local Btn = Instance.new("TextButton", Box)
    Btn.Size = UDim2.new(0.5, -10, 0, 38)
    Btn.Position = UDim2.new(0.5, 0, 0.5, -19)
    ...
    DropScroll.CanvasSize = UDim2.fromOffset(0, #options * 38)
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        if isOpen then
            ...
            DropScroll.Position = UDim2.fromOffset(btnPos.X, btnPos.Y + 42)
            DropScroll.Size = UDim2.new(0, 200, 0, math.min(#options * 38, 250))
        end
    end)
...
-- =========================
-- FUNÇÕES AUXILIARES PARA ABA PRINCIPAL
-- =========================
...
local function InfoBox(parent, title, value, color)
    ...
    Value.Size = UDim2.new(1, -20, 0.5, 0)
...
local function StatusIndicator(parent, title, isActive)
    local Box = Instance.new("Frame", parent)
    ...
    Indicator.Position = UDim2.fromOffset(10, 17)
...
-- CRIANDO ABAS
-- =========================
local PrincipalTab = CriarAba("🏠 Principal")  
...
-- Definir aba Principal como primeira
for _, v in pairs(Tabs) do
    v.Frame.Visible = false
    Tween(v.Btn, 0.2, {BackgroundColor3 = Config.Tema.Botao})
end
Tabs["🏠 Principal"].Frame.Visible = true
Tween(Tabs["🏠 Principal"].Btn, 0.2, {BackgroundColor3 = Config.Tema.Ativo})

-- =========================
-- ABA PRINCIPAL
-- =========================
Section(PrincipalTab, "Comunidade Discord")

Button(PrincipalTab, "🎮 Entrar no Discord", function()
    setclipboard("https://discord.gg/RnZ6XHHFj7")
    print("✅ Link copiado: https://discord.gg/RnZ6XHHFj7")
    print("📋 Cole no seu navegador!")
end)

local DiscordInfo = Instance.new("Frame", PrincipalTab)
DiscordInfo.Size = UDim2.new(1, 0, 0, 80)
DiscordInfo.BackgroundColor3 = Config.Tema.Secundario
DiscordInfo.BackgroundTransparency = 0.3
DiscordInfo.BorderSizePixel = 0

Instance.new("UICorner", DiscordInfo).CornerRadius = UDim.new(0, 10)

local DiscordText = Instance.new("TextLabel", DiscordInfo)
DiscordText.Size = UDim2.new(1, -20, 1, -20)
DiscordText.Position = UDim2.fromOffset(10, 10)
DiscordText.Text = "💬 Discord Lag Teck\n👥 Membros ativos e suporte\n🎯 Updates e novidades"
DiscordText.BackgroundTransparency = 1
DiscordText.TextColor3 = Config.Tema.Texto
...
Section(PrincipalTab, "Informações de Tempo")

local UptimeBox = InfoBox(PrincipalTab, "⏱️ Tempo do Servidor", GetServerUptime(), Config.Tema.AzulClaro)
local SessionBox = InfoBox(PrincipalTab, "🕐 Sua Sessão", GetSessionTime(), Color3.fromRGB(0, 255, 0))

task.spawn(function()
    while task.wait(60) do
        pcall(function()
            UptimeBox.ValueLabel.Text = GetServerUptime()
            SessionBox.ValueLabel.Text = GetSessionTime()
        end)
    end
end)
...
local MoneyBox = InfoBox(PrincipalTab, "💰 Dinheiro", "Carregando...", Color3.fromRGB(255, 215, 0))
local FragBox = InfoBox(PrincipalTab, "💎 Fragmentos", "Carregando...", Color3.fromRGB(100, 200, 255))
...
task.spawn(function()
    while task.wait(5) do
        pcall(function()
            ...
            MoneyBox.ValueLabel.Text = "$ " .. FormatNumber(money)
            FragBox.ValueLabel.Text = FormatNumber(frags)
        end)
    end
end)

Section(PrincipalTab, "Status de Eventos (SEA 2 e 3)")

local StatusGrid = Instance.new("Frame", PrincipalTab)
StatusGrid.Size = UDim2.new(1, 0, 0, 200)
...
local LeviathanStatus = StatusIndicator(StatusGrid, "🐉 Leviathan Spawned", false)
local MirageStatus = StatusIndicator(StatusGrid, "🏝️ Ilha Miragem", false)
local VolcanoStatus = StatusIndicator(StatusGrid, "🌋 Ilha Vulcão", false)
local KatakuriStatus = StatusIndicator(StatusGrid, "🍩 Katakuri Progress", false)
...
Toggle(FarmTab, "Auto Farm Level", function(v)
    if v and _G.Settings.WeaponType == "Nenhum" then
        warn("⚠️ SELECIONE UM TIPO DE ARMA!")
        return
    end
...
Button(FruitTab, "📊 Loja Normal", function()
    task.spawn(function()
        local remote = GetRemote()
...
Button(FruitTab, "🌀 Loja Miragem", function()
    task.spawn(function()
        local remote = GetRemote()
        if not remote then return end
        
        local success = pcall(function()
            return remote:InvokeServer("GetFruits", LocalPlayer.Character.PrimaryPart.CFrame, true)
        end)
        
        if success then
            print("✅ Loja miragem verificada!")
        end
    end)
end)
...
for i = 1, 3 do
    ...
    SeaBtn.Size = UDim2.new(0.3, 0, 0, 50)
    SeaBtn.Text = "SEA " .. i
    ...
    local SeaBtnCorner = Instance.new("UICorner", SeaBtn)
    SeaBtnCorner.CornerRadius = UDim.new(0, 8)
...
local function OpenHub()
    ...
    local targetSize = IsMobile() and UDim2.fromScale(0.9, 0.8) or UDim2.fromScale(0.75, 0.7)
